    ...
    def fn(rec: SentenceRecord) -> str:
        ...
        for sim, meta in neighbors:
            ...
            if uid in translate_cache:
                ...
                if sim >= retrieval_cfg.reuse_threshold and not rec.sentence_translation_en:
        ...
        for i, l in enumerate(ctx_lines, 1):
            if used + len(l) > retrieval_cfg.max_context_chars:
                break
...
async def run_translation(
    ...
    translate_cache = {k: v for k, v in existing.items() if v.get("sentence_translation_en")}
    ...
    async with httpx.AsyncClient() as client:
        ...
        for i in tqdm(range(0, len(records), batch_size), desc="Translating"):
            ...
            untranslated = []
            for r in batch:
                if r.metadata["uuid"] in existing and existing[r.metadata["uuid"]].get("sentence_translation_en"):
                    continue
